...
export function Sidebar({ engagementId, engagementName, athleteName }: SidebarProps) {
  ...
  return (
    <div>
      {/* Mobile: compact horizontal bar, not the fixed-width vertical sidebar */}
      <div className="flex md:hidden flex-col" style={{ background: 'var(--cream)', borderBottom: '1px solid var(--line)' }}>
        <div className="flex items-center justify-between px-4 py-2.5" style={{ borderBottom: '1px solid var(--line)' }}>
          <Link
            ...
            className="flex items-center gap-1.5 text-[11px] font-semibold tracking-[0.12em] uppercase shrink-0"
          >
            <ArrowLeft size={11} strokeWidth={2.4} />
            Searches
          </Link>
          <div style={{ minWidth: 0 }} className="text-right ml-3">
            <div style={{ fontFamily: 'var(--display)', color: 'var(--navy)' }} className="text-[14px] font-medium truncate">
              {engagementName ?? 'Portal Search'}
            </div>
            {athleteName && (
              <div style={{ color: 'var(--slate-soft)' }} className="text-[11px] truncate">{athleteName}</div>
            )}
          </div>
        </div>
        <nav className="flex overflow-x-auto" aria-label="Recruitment stages">
          {STEPS.map((step, i) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-2 px-4 py-3 whitespace-nowrap shrink-0"
                style={{
              ...
              >
                ...
                >
                  {isDone ? <Check size={10} strokeWidth={2.8} /> : i + 1}
                </div>
                {step.label}
              </Link>
            )
